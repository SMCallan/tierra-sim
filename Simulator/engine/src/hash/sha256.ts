...
export type Sha256Function = (value: string) => string;
...
/** Browser-safe, deterministic SHA-256 over a UTF-8 string. */
export function sha256(value: string): string {
  ...
  for (let chunkStart = 0; chunkStart < message.length; chunkStart += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      schedule[index] =
        (((message[offset] as number) << 24) |
          ((message[offset + 1] as number) << 16) |
          ((message[offset + 2] as number) << 8) |
          (message[offset + 3] as number)) >>>
        0;
    }
